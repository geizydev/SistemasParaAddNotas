...
public class ViewExibirMaterial extends JFrame{
	...
	public ViewExibirMaterial(SistemaGeral objSistema) {
		super("Materiais");
		setLayout(null);
		...
		if (objSistema.getListaMateriais().size() > 0) {
			desenharMateriais();
		}
	}
	
	public void desenharMateriais() {
		//validate();
		//repaint();
		System.out.println("ghj");
		exibirPanel.setIcon(objSistema.getListaMateriais().get(indiceMaterial));
	}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(e.getSource()==avancar) {
				if (e.getSource() == avancar) {
					...
					if (indiceMaterial < objSistema.getListaMateriais().size()-1) {
						indiceMaterial = indiceMaterial + 1;
						desenharMateriais();
					}
				}
